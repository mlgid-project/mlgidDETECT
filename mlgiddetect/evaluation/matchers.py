...
class Matcher(object):
    def __call__(self, target: Tensor, predicted: Tensor) -> Tuple[ndarray, ndarray, ndarray]:
        raise NotImplementedError


class IoUMatcher(Matcher):
    def __init__(self, iou: float = 0.1):
        self.iou = iou

    def __call__(self, target: Tensor, predicted: Tensor) -> Tuple[ndarray, ndarray, ndarray]:
        return get_iou_match(target, predicted, self.iou)


class QMatcher(Matcher):
    def __init__(self, thresh: float = 10., rel: bool = False, min_iou: float = 0.01):
        self.thresh = thresh
        self.rel = rel
        self.min_iou = min_iou

    def __call__(self, target: Tensor, predicted: Tensor) -> Tuple[ndarray, ndarray, ndarray]:
        return get_q_match(target, predicted, thresh=self.thresh, rel=self.rel, min_iou=self.min_iou)


def get_matcher(match_criterion: str, *args, **kwargs) -> Matcher:
    if match_criterion == 'iou':
        return IoUMatcher(*args, **kwargs)
    if match_criterion == 'q':
        kwargs['rel'] = False
        return QMatcher(*args, **kwargs)
    if match_criterion == 'q_rel':
        kwargs['rel'] = True
        return QMatcher(*args, **kwargs)
...
def calc_box_dq_mtx(target, predicted, rel: bool = False, min_iou: float = 1e-6, **kwargs):
    iou_mtx = box_iou(target, predicted).cpu().numpy()
